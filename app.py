...
def calculate_last_edit():
    last_edit_date = datetime.datetime.fromtimestamp(
        db_path.stat().st_mtime).date()
    global days_since_edit
...
class Atest_window(QWidget):
    def __init__(self):
        ...
        self.fs_edit.setValidator(
            QRegularExpressionValidator(r"FS .*\/[0-9]{4}"))
        ...
        self.last_edit_text = QLabel(
            f'Dni od ostatniej edycji parametrów: {days_since_edit}')
        if days_since_edit >= 20:
            self.last_edit_text.setStyleSheet(self.rejected_style)
        layout.addWidget(self.last_edit_text, 3, 0)
        ...
        self.response_text = QLabel(
            "Wygeneruj najnowszy", alignment=Qt.AlignBottom)
    ...
    def validate_document(self, name) -> bool:
        if not re.match(r"FS [0-9]+\/[0-9]{4}", name):
            self.response_text.setText(
                f"Wpisz poprawnny nr faktury (np. FS 123/{today_date.year})")
            self.response_text.setStyleSheet(self.rejected_style)
            return False
        else:
            self.response_text.setText(
                f"Wygeneruj atest dla {self.document_id}")
            self.response_text.setStyleSheet(self.accept_style)
            return True
    ...
    def edit_param(self):
        ...
        self.w.window_closed.connect(self.refresh)
        self.w.show()

    @QtCore.Slot()
    def refresh(self):
        self.last_edit_text.setText(
            f'Dni od ostatniej edycji parametrów: {days_since_edit}')
...
class Params_window(QWidget):
    ...
    params_df.iloc[2] = params_df.iloc[2].apply(
        lambda x: str(x).replace('.', ','))
    params_df.iloc[3] = params_df.iloc[3].apply(
        lambda x: str(x).replace('.', ','))
    params_df.iloc[4] = params_df.iloc[4].apply(
        lambda x: str(x).replace('.', ','))
