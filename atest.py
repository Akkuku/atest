...
class Atest():
    def __init__(self, atest_path, docs_path, db_path, document_id=None):
        ...
        fs_df = pd.read_excel(
            str(docs_path/self.document_id.replace('/', '_')) + '.xls')
        ...
        self.assortment['Ilość'] = self.assortment['Ilość'].apply(
            lambda x: int(''.join(x.split()[:-1])))
        self.assortment = self.assortment.sort_values(
            by='Ilość', ascending=False)
        self.assortment['Ilość'] = self.assortment['Ilość'].apply(
            lambda x: str(x) + ' kg')

        self.assortment['Asortyment'] = self.assortment['Asortyment'].replace(
            regex=r'Otręby pszenne.*', value='Otręby pszenne')
        self.client = fs_df.iloc[5, 11].split('\n')[0].strip()
        split_client = self.client.split(' ')

        if len(split_client) > 4:
            self.client = ' '.join(
                split_client[:4]) + '\n' + ' '.join(split_client[4:])
    ...
    def make_pdf(self):
        ...
        param_df.iloc[2] = param_df.iloc[2].apply(
            lambda x: str(x).replace('.', ','))
        param_df.iloc[3] = param_df.iloc[3].apply(
            lambda x: str(x).replace('.', ','))
        param_df.iloc[4] = param_df.iloc[4].apply(
            lambda x: str(x).replace('.', ','))
        ...
        doc = SimpleDocTemplate(str(
            self.atest_path / f'Atest {self.document_id.replace("/","_")}.pdf'), pagesize=A4, bottomMargin=0.5*inch)
        ...
        try:
            doc.build(elements)
            subprocess.Popen(
                [self.atest_path / f"Atest {self.document_id.replace('/','_')}.pdf"], shell=True)
            return True
        except IOError:
